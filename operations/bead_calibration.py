...
class BeadCalibrationOp(HasStrictTraits):
    ...
    def estimate(self, experiment,are_peaks_values_provided = False,provided_peak_mefs_all_channels = {}): 
        """
        ...
        if not are_peaks_values_provided:
        ...
        # find peaks
        for channel in channels:
            if are_peaks_values_provided:
                self._peaks[channel] = provided_peak_mefs_all_channels[channel]
            else:
        ...
        for channel in channels:
            ...
            # "mef = mean equivalent fluorochrome"
            if are_peaks_values_provided:
                if not mef_unit in provided_peak_mefs_all_channels:
                    raise util.CytoflowOpError('units',
                                           "Invalid unit {0} specified for channel {1}".format(mef_unit, channel))

                provided_peak_mefs = provided_peak_mefs_all_channels[mef_unit]
                if len(peaks) != len(provided_peak_mefs):
                    raise util.CytoflowOpError(None,
                                           "Length of peak values provided does not match number of found peak"
                                           "check the diagnostic plot"
                                           .format(channel))

                # linear regression of the peak locations against mef subset
                lr = np.polyfit(np.log10(peaks), 
                                    np.log10(provided_peak_mefs), 
                                    deg = 1, 
                                    full = True)
                ...
                self._mefs[channel] = provided_peak_mefs
                a = lr_parameters[0]
                b = 10 ** lr_parameters[1]

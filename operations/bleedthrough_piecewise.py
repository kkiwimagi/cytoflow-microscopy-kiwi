...
class BleedthroughPiecewiseOp(HasStrictTraits):
    ...
    def estimate(self, experiment, subset = None): 
        ...
        if ( self.controls_frames != {} ):
            self._channels = list(self.controls_frames.keys())
        else:
        ...
        for channel in self._channels:
            ...
            arr = tube_data[channel].values
            zz = [x < y for x,y in zip(arr, arr[1:])]
            f = (arr[:-1])[zz]
            ...
            for to_channel in self._channels:
                ...
                t = tube_data[to_channel].values
                t = (t[:-1])[zz]
                
                self._splines[from_channel][to_channel] = \
                    scipy.interpolate.LSQUnivariateSpline(f, #tube_data[from_channel].values,
                                                          t, #tube_data[to_channel].values,
                                                          t = knots,
                                                          k = 1)
    ...
    def default_view(self, **kwargs):
        ...
        if (set(self.controls.keys()) != set(self._splines.keys())) and (set(self.controls_frames.keys()) != set(self._splines.keys())):
            raise util.CytoflowOpError('controls',
    ...
    def plot(self, experiment = None, **kwargs):
        ...
        for from_idx, from_channel in enumerate(channels):
            for to_idx, to_channel in enumerate(channels):
                ...
                ax = plt.subplot(num_channels, 
                            num_channels, 
                            from_idx + (to_idx * num_channels) + 1)
                xscale_mpl_params = xscale.get_mpl_params(ax.xaxis)
                yscale_mpl_params = yscale.get_mpl_params(ax.yaxis)
                plt.xscale('log', **xscale_mpl_params)
                plt.yscale('log', **yscale_mpl_params)
